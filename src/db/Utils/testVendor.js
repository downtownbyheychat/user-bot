import { getVendorByName, getVendorById, searchItemAcrossVendors, getVendorCatalogue, validateOrderItem, getAllVendors, has_mixed_types } from './vendor.js';
...
//   allVendors.forEach(v => console.log(`   - ${v.name}`));
  ...
  // Test 7: has mixed types
    ...
    const hasMixedTypes = await has_mixed_types(vendor.id, mixedItems);

    console.log(hasMixedTypes ? '✅ Mixed quantity types detected' : '❌ No mixed quantity types');
    console.log('');
